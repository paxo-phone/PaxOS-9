...
void testCommand(
    std::string commandText, bool shellMode,
    Command::CommandType expectedCommandType, std::string expectedOutput
) {
    char info_input[INPUT_MAX_SIZE] = {'\0'};
    strcpy(info_input, commandText.c_str());
    Command command = Command(info_input);
    ...
    if (CommandsManager::defaultInstance->shellMode) {
        ...
        if (text.size() >= 12) {
            // 4 begin bytes
            ASSERT_TRUE(
                text[0] == 0xff && text[1] == 0xfe && text[2] == 0xfd &&
                text[3] == 0xfc
            );
            ...
            expectedOutputHash = (text[11] << 24) | (text[10] << 16) |
                                 (text[9] << 8) | text[8]; // little endian

            text.erase(
                0,
                12
            ); // remove the size and hash from the expected output
    ...
    if (result != 0) {
        std::cout << "Got shell output: " << text << " expected "
        ...
        for (size_t i = 0; i < std::max(textSize, expectedOutputSize); i++) {
            if (i < textSize && i < expectedOutputSize) {
                std::cout << "Binary shell output: "
                          << std::bitset<8>(text.c_str()[i]) << "/"
                          << std::bitset<8>(expectedOutput.c_str()[i])
                          << std::endl;
                std::cout << "char shell output: " << text.c_str()[i] << "/"
                          << expectedOutput.c_str()[i] << std::endl;
                if (std::bitset<8>(text.c_str()[i]) !=
                    std::bitset<8>(expectedOutput.c_str()[i])) {
        ...
        std::cout << "Arguments: " << command.arguments[0] << " with size "
                  << std::string(command.arguments[0]).size() << "; "
                  << command.arguments[1] << " with size "
                  << std::string(command.arguments[1]).size() << "; "
