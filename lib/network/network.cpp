...
namespace Network
{
    ...
    void NetworkManager::submitRequest(std::shared_ptr<Request> request) {
        ...
        std::cout << "[" << NETWORK_TAG << "] Queued request ID " << request->id << " for URL: " << request->url << std::endl;
    }
    ...
    void NetworkManager::processQueue() {
        ...
        if (m_currentRequest) {
            std::cout << "[" << NETWORK_TAG << "] Processing request ID " << m_currentRequest->id << "..." << std::endl;
    ...
    void NetworkManager::_executeRequest(std::shared_ptr<Request> request) {
        ...
        switch (interface) {
            case NetworkInterface::WIFI:
                std::cout << "[" << NETWORK_TAG << "] Routing request ID " << request->id << " via Wi-Fi." << std::endl;
                _executeWifiRequest(request);
                break;
            case NetworkInterface::CELLULAR:
                std::cout << "[" << NETWORK_TAG << "] Routing request ID " << request->id << " via Cellular." << std::endl;
                _executeCellularRequest(request);
                break;
            case NetworkInterface::NONE:
            default:
                std::cout << "[ERROR:" << NETWORK_TAG << "] No route available for request ID " << request->id << "." << std::endl;
    ...
    void NetworkManager::_completeCurrentRequest(NetworkStatus status) {
        if (m_currentRequest) {
            std::cout << "[" << NETWORK_TAG << "] Completing request ID " << m_currentRequest->id << " with status " << static_cast<int>(status) << "." << std::endl;
    ...
    esp_err_t NetworkManager::_httpEventHandler(esp_http_client_event_t* evt) {
        ...
        switch(evt->event_id) {
            ...
            case HTTP_EVENT_ON_HEADER:
                ...
                std::cout << "[" << NETWORK_TAG << "] HTTP_EVENT_ON_HEADER, key=" << evt->header_key << ", value=" << evt->header_value << std::endl;
                break;
            case HTTP_EVENT_ON_DATA:
                std::cout << "[" << NETWORK_TAG << "] HTTP_EVENT_ON_DATA, len=" << evt->data_len << std::endl;
                if (request->on_data) {
    ...
    static size_t header_callback(char* buffer, size_t size, size_t nitems, void* userdata) {
        ...
        if (header.rfind("HTTP/", 0) == 0) {
            long status_code = 0;
            curl_easy_getinfo(static_cast<CURL*>(request->internal_handle), CURLINFO_RESPONSE_CODE, &status_code);
    ...
    void NetworkManager::disconnectWifi() {
        std::cout << "[" << NETWORK_TAG << "] Disconnecting from Wi-Fi on PC (simulation)." << std::endl;
        m_isWifiConnected = false;
    }
    ...
    void NetworkManager::_executeWifiRequest(std::shared_ptr<Request> request) {
        ...
        if (res != CURLE_OK) {
            std::cout << "[ERROR:" << NETWORK_TAG << "] curl_easy_perform() failed: " << curl_easy_strerror(res) << std::endl;
            _completeCurrentRequest(NetworkStatus::REQUEST_FAILED);
        } else {
