...
Command::Command(char (&input)[INPUT_MAX_SIZE]) {
    ...
    if (pos == -1 && !shellMode) {
        ...
        if (!found) {
            SerialManager::sharedInstance->commandLog(NON_SHELL_MODE_ERROR_CODE +
                                                      std::string("ERROR: NO HEADER FOUND"));
            return;
        }
    }
    ...
    // match the command_type with a loop with the CommandType enum but don't forget that the input
    // is a 16 char array
    ...
    if (this->type == CommandType::unknown) {
        if (shellMode) {
            ...
            SerialManager::sharedInstance->commandLog(NON_SHELL_MODE_ERROR_CODE +
                                                      std::string("ERROR: UNKONWN COMMAND TYPE") +
                    ...
                    if (isEscaping) {
                        ...
                        if (shellMode) {
                            SerialManager::sharedInstance->commandLog(
                                "ERROR: UNEXPECTED QUOTE CHARACTER AT POSITION " +
                                std::to_string(i));
                        } else {
                            SerialManager::sharedInstance->commandLog(
...
std::unordered_map<Command::CommandType, std::string> Command::command_types_raw_strings{
    {Command::CommandType::sm, "sm"},           {Command::CommandType::console, "console"},

    {Command::CommandType::info, "info"},       {Command::CommandType::echo, "echo"},
    {Command::CommandType::apps, "apps"},       {Command::CommandType::files, "files"},
    {Command::CommandType::elevate, "elevate"}, {Command::CommandType::lte, "lte"},

    {Command::CommandType::ls, "ls"},           {Command::CommandType::touch, "touch"},
    {Command::CommandType::mkdir, "mkdir"},     {Command::CommandType::rm, "rm"},
    {Command::CommandType::cp, "cp"},           {Command::CommandType::mv, "mv"},
    {Command::CommandType::cat, "cat"},         {Command::CommandType::download, "download"},
