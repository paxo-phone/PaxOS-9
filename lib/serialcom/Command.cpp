...
Command::Command(char (&input)[INPUT_MAX_SIZE]) {
    ...
    for (size_t i = 0; i < MAX_COMMMAND_TYPE_SIZE; i++) {
        if (input[current_index + i] == ' ' ||
            input[current_index + i] == '\0') {
            command_type[i] = '\0';
            command_size = i;
            break;
        }
    ...
    for (size_t argument_index = 0;
         argument_index < MAX_COMMAND_ARGUMENTS_COUNT;
         argument_index++) {
        // skip spaces
        ...
        for (size_t i = current_index; i < current_index + MAX_ARGUMENT_SIZE;
             i++) {
            if (input[i] == '\0' || input[i] == '\n') {
                ...
                if (isUsingQuotes) {
                    ...
                    if (isEscaping) {
                        ...
                        this->type = CommandType::unknown;
                        for (size_t j = 0; j < MAX_COMMAND_ARGUMENTS_COUNT;
                             j++) {
...
std::unordered_map<Command::CommandType, std::string>
    Command::command_types_raw_strings{
