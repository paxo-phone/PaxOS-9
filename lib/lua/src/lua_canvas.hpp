...
class LuaCanvas : public LuaWidget {
  public:
    ...
    void drawRoundRect(
        int16_t x, int16_t y, uint16_t w, uint16_t h, uint16_t radius,
        color_t color
    ) {
    ...
    void drawPolygon(sol::table vertices, color_t color) {
        ...
        for (std::vector<int16_t> vertex :
             convert_sequence<std::vector<int16_t>>(vertices)) {
