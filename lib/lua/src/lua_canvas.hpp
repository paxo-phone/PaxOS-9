...
class LuaCanvas : public LuaWidget {
  public:
    ...
    void drawLine(int16_t x1, int16_t y1, int16_t x2, int16_t y2, color_t color) {
        widget->drawLine(x1, y1, x2, y2, color);
    }

    void drawText(int16_t x, int16_t y, std::string text, color_t color) {
        widget->drawText(x, y, text, color);
    }
    ...
    void drawTextCenteredInRect(int16_t x, int16_t y, uint16_t w, uint16_t h, std::string text,
        ...
        widget->drawTextCenteredInRect(x, y, w, h, text, color, horizontallyCentered,
                                       verticallyCentered);
    }

    void onTouch(sol::protected_function function) {
        this->onTouchFunc = function;
    }
