...
std::string latin1HexToUtf8(const std::string& hexString) {
    ...
    for (size_t i = 0; i < hexString.length(); i += 2) {
        ...
        char byte = static_cast<char>((hexCharToInt(hexString[i]) << 4) | hexCharToInt(hexString[i+1]));
...
PDU decodePDU(std::string pdu)
{
    ...
    char PDU_mode = hex_to_int(pdu.substr(i, 2)) - 2; i += 2;

    // std::cout << "PDU_mode: " << int(PDU_mode) << " " << std::bitset<8>(PDU_mode).to_string() << std::endl;
    ...
    // std::cout << "TP-MTI: " << (int) getBit(PDU_mode, 0) << (int) getBit(PDU_mode, 1) << std::endl;
