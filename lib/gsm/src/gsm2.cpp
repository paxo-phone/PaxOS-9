...
namespace Gsm
{
    ...
    static std::string gsm7SeptetToUtf8(unsigned char septet) {
        ...
        switch (septet) {
            ...
            case 0x5E: return "\xC3\x84"; /* Ä */ case 0x5F: return "_";
            case 0x60: return "\xC3\x9F";
            ...
            case 0x7B: return "\xC3\xA4";
            case 0x7C: return "\xC3\xB6";
            case 0x7D: return "\xC3\xBC";
            case 0x7E: return "a";
            case 0x7F: return "\xE2\x82\xAC"; /* € */
            default:
                return "?";
        }
    }
    ...
    static char gsm7SeptetToChar(unsigned char septet) {
        switch (septet) {
            ...
            case 0x1A: return '?';
            case 0x1B: return ' ';
            case 0x1C: return '\xC6';
            case 0x1D: return '\xE6';
            case 0x1E: return '\xDF';
            case 0x1F: return '\xC9';
            case 0x20: return ' ';
            default:
    ...
    static bool decodePduDeliver(const std::string& pdu_hex_string, DecodedPdu& result) {
        try {
            ...
            if (tp_mti == 0x00) {
                ...
                if (dcs_group == 0x00) {
                     if ((tp_dcs & 0x20) == 0x00) {
                        if (dcs_char_set == 0x00) is_7bit_encoding = true;
                        else if (dcs_char_set == 0x01) is_8bit_encoding = true;
                        else if (dcs_char_set == 0x02) is_ucs2_encoding = true;
                     } else {
        ...
        if (decodePduDeliver(pdu_hex_string, decoded_sms)) {
            ...
            if (decoded_sms.type == SmsType::TEXT_SMS || decoded_sms.type == SmsType::MMS_NOTIFICATION) {
                if (decoded_sms.type == SmsType::MMS_NOTIFICATION) {
                    ...
                    conv.messages.push_back({decoded_sms.messageContent, true, std::to_string(getCurrentTimestamp())});
    ...
    void processURC(std::string data)
    {
        if (data.find("RING") == 0) {
            ...
            std::cout << "[HTTP URC] Method: " << method_urc << ", Status: " << statusCode << ", Data Length: " << dataLen << std::endl;
            ...
            if (statusCode >= 200 && statusCode < 400) { // Handle success and redirects
                ...
                std::cout << "[HTTP URC] Request failed with status code: " << statusCode << std::endl;
    ...
    static std::string pack7Bit(const std::string& message, int& septetCount) {
        ...
        for (char c_char : message) {
            ...
            else if (c_char == '\xE8') septet = 0x04;
            else if (c_char == '\xE9') septet = 0x05;
            else if (c_char == '\xF9') septet = 0x06;
            else if (c_char == '\xEC') septet = 0x07;
            else if (c_char == '\xF2') septet = 0x08;
            else if (c_char == '\xC7') septet = 0x09;
            else if (c_char == '\n') septet = 0x0A;
            else if (c_char == '\xD8') septet = 0x0B;
            else if (c_char == '\xF8') septet = 0x0C;
            else if (c_char == '\r') septet = 0x0D;
            else if (c_char == '\xC5') septet = 0x0E;
            else if (c_char == '\xE5') septet = 0x0F;
            else if (c_char == '_') septet = 0x11;
            else if (c_char == '\xC6') septet = 0x1C;
            else if (c_char == '\xE6') septet = 0x1D;
            else if (c_char == '\xDF') septet = 0x1E;
            else if (c_char == '\xC9') septet = 0x1F;
    ...
    void httpRequest(HttpRequest request) {
        ...
        // POST-specific part
        if (currentHttpRequestDetails->method == HttpMethod::POST && !currentHttpRequestDetails->body.empty()) {
            std::cout << "[HTTP] POST method detected, preparing body." << std::endl;
            auto setDataReq = std::make_shared<Request>();
            setDataReq->command = "AT+HTTPDATA=" + std::to_string(currentHttpRequestDetails->body.length()) + ",10000";
        ...
        actionReq->command = (currentHttpRequestDetails->method == HttpMethod::POST) ? "AT+HTTPACTION=1" : "AT+HTTPACTION=0";
        ...
        while ((lineEndPos = lineBuffer.find('\n')) != std::string::npos)
        {
            ...
            if (line.rfind("+HTTPREAD:", 0) == 0)
            {
                ...
                if (datasize > 0) {
                    ...
                    std::cout << "[DEBUG] Starting to read data, expected size: " << datasize << std::endl;
                    ...
                    if (bytesRead == datasize) {
                        if (currentHttpRequestDetails && currentHttpRequestDetails->on_data) {
                            ...
                            std::cout << "[DEBUG] Data chunk received, invoking on_data callback" << std::endl;
                            currentHttpRequestDetails->on_data(data_chunk);
                            httpBytesRead += bytesRead;
                            std::cout << "[DEBUG] Total HTTP bytes read so far: " << httpBytesRead << std::endl;
                        }
                    } else {
                ...
                if(line.find("OK") != std::string::npos) {
                    ...
                    std::cout << "[HTTP] HTTPREAD error, total bytes read: " << httpBytesRead << std::endl;
                    _completeHttpRequest(HttpResult::READ_ERROR);
                }
            }
