...
#define myCalculOfDay(d)                                                       \
    (1461 * f_Date(d.annee, d.mois) / 4 + 153 * g_Date(d.mois) / 5 + d.jour)
...
); // Check if quality has been successfully read at least once
...
);                  // The offset reported by modem (zz in +/-zz)
