...
inline uint32_t make32L(
    const uint8_t v1, const uint8_t v2, const uint8_t v3, const uint8_t v4
) {
...
imgdec::IMGData imgdec::decodeHeader(const uint8_t* rawData) {
    ...
    if (rawData[0x00] == 'B' && rawData[0x01] == 'M') {
        ...
        imgData.width =
            make32L(rawData[0x12], rawData[0x13], rawData[0x14], rawData[0x15]);
        imgData.heigth =
            make32L(rawData[0x16], rawData[0x17], rawData[0x18], rawData[0x19]);
    } else if (rawData[0x01] == 'P' && rawData[0x02] == 'N' &&
               rawData[0x03] == 'G') {
        imgData.type = PNG;

        imgData.width =
            make32B(rawData[0x10], rawData[0x11], rawData[0x12], rawData[0x13]);
        imgData.heigth =
            make32B(rawData[0x14], rawData[0x15], rawData[0x16], rawData[0x17]);
    } else {
        imgData.type = JPG;
        ...
        // an error
        if (i >= 2000) {
            std::cerr << "Invalid JPEG file: SOF0 marker not found."
                      << std::endl;
        }
    }
    /*else
...
void imgdec::encodeJpg(
    const uint8_t* rawData, uint32_t width, uint32_t height,
